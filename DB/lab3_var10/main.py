...
models_name = {
    'customers': table_models.Customers,
    'manufacturers': table_models.Manufacturers,
    'video_cards': table_models.VideoCards,
    'vendors': table_models.Vendors,
    'vendors_videocards': table_models.VendorsVideocards,
    'customers_videocards': table_models.CustomersVideocards
    }
...
def main():
    ...
    main_menu(database)
    # database.update_table(table_models.Customers)
...
def select_menu(database):
    ...
    if table_name not in database.table_names:
        ...
        view.clear()
        view.print_table(database.get_from_table(models_name.get(table_name)),
                         table_name)
        return True
...
def update_menu(database):
    ...
    if table_name not in database.table_names:
        ...
        table = models_name.get(table_name)
        view.print_column_names(database, table)
        column_name = input('Input column name: ')
        
        if column_name not in database.get_column_names(table):
            return False
        else:
            value = input('Input value: ')
            update_id = input('Input id: ')
            database.update_table(table, column_name, value, update_id)
            return True
...
def text_search_menu(database):
    ...
    if view.print_table(database.full_text_search(search_str)):
        return True
    else:
